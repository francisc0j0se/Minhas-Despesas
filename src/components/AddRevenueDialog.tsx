...
const revenueSchema = z.object({
  ...
  account_id: z.string().uuid("A conta é obrigatória para receitas."),
...
const AddRevenueDialog = ({ isOpen, onOpenChange }: AddRevenueDialogProps) => {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<z.infer<typeof revenueSchema>>({
  ...
  const addRevenueMutation = useMutation({
    mutationFn: async (data: z.infer<typeof revenueSchema>) => {
  ...
  const onSubmit = (data: z.infer<typeof revenueSchema>) => {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={addRevenueMutation.isPending}>
              {addRevenueMutation.isPending ? "Salvando..." : "Salvar Receita"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
